...
### 前提MODコードの参照方法
- 前提MODの実装を調べる際はCode Context MCPを使うのが最も効率的です。`mcp__code-context__search_code`で目的のAPIやクラスを検索し、必要に応じて`mcp__code-context__get_indexing_status`でインデックス状態を確認してから利用してください。
- Code Context MCP経由での検索はローカルプロジェクトのファイル移動よりも高速に目的コードへ到達でき、差分確認や関連箇所の洗い出しにも役立ちます。
